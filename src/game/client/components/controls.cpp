...
void CControls::OnReset( )
{
	...
	for ( int &AmmoCount : m_aAmmoCount ) AmmoCount = 0;
...
void CControls::ResetInput( int Dummy )
{
	...
	if ( ( m_aLastData[ Dummy ].m_Fire & 1 ) != 0 ) m_aLastData[ Dummy ].m_Fire++;
...
static void ConKeyInputState( IConsole::IResult *pResult, void *pUserData )
{
	...
	if ( pState->m_pControls->GameClient( )->m_GameInfo.m_BugDDRaceInput && pState->m_pControls->GameClient( )->m_Snap.m_SpecInfo.m_Active ) return;

	if ( g_Config.m_ClDummy ) *pState->m_pVariable2 = pResult->GetInteger( 0 );
	else *pState->m_pVariable1 = pResult->GetInteger( 0 );
}
...
static void ConKeyInputCounter( IConsole::IResult *pResult, void *pUserData )
{
	...
	int *v;
	if ( g_Config.m_ClDummy ) v = pState->m_pVariable2;
	else v = pState->m_pVariable1;

	if ( ( ( *v ) & 1 ) != pResult->GetInteger( 0 ) ) ( *v )++;
	*v &= INPUT_STATE_MASK;
}
...
static void ConKeyInputSet( IConsole::IResult *pResult, void *pUserData )
{
	...
	{
		if ( g_Config.m_ClDummy ) *pSet->m_pVariable2 = pSet->m_Value;
		else *pSet->m_pVariable1 = pSet->m_Value;
	}
}
...
void CControls::OnMessage( int Msg, void *pRawMsg )
{
	if ( Msg == NETMSGTYPE_SV_WEAPONPICKUP )
	{
		...
		if ( g_Config.m_ClAutoswitchWeapons ) m_aInputData[ g_Config.m_ClDummy ].m_WantedWeapon = pMsg->m_Weapon + 1;
		// We don't really know ammo count, until we'll switch to that weapon, but
		// any non-zero count will suffice here
...
int CControls::CurrentTeeSnapInput( int *pData )
{
	...
	bool	       Send	    = false;
	// update player state
	if ( m_pClient->m_Chat.IsActive( ) ) m_aInputData[ g_Config.m_ClDummy ].m_PlayerFlags = PLAYERFLAG_CHATTING;
	else if ( m_pClient->m_Menus.IsActive( ) ) m_aInputData[ g_Config.m_ClDummy ].m_PlayerFlags = PLAYERFLAG_IN_MENU;
	else m_aInputData[ g_Config.m_ClDummy ].m_PlayerFlags = PLAYERFLAG_PLAYING;

	if ( m_pClient->m_Scoreboard.Active( ) ) m_aInputData[ g_Config.m_ClDummy ].m_PlayerFlags |= PLAYERFLAG_SCOREBOARD;

	if ( m_pClient->m_Controls.m_aShowHookColl[ g_Config.m_ClDummy ] ) m_aInputData[ g_Config.m_ClDummy ].m_PlayerFlags |= PLAYERFLAG_AIM;

	if ( m_aLastData[ g_Config.m_ClDummy ].m_PlayerFlags != m_aInputData[ g_Config.m_ClDummy ].m_PlayerFlags ) Send = true;
	...
	{
		if ( !GameClient( )->m_GameInfo.m_BugDDRaceInput ) ResetInput( g_Config.m_ClDummy );
		...
		if ( time_get( ) > LastSendTime + time_freq( ) ) Send = true;
	}
	else
	{
		m_aInputData[ g_Config.m_ClDummy ].m_TargetX = ( int )m_aMousePos[ g_Config.m_ClDummy ].x;
		...
		if ( m_aInputDirectionLeft[ g_Config.m_ClDummy ] && !m_aInputDirectionRight[ g_Config.m_ClDummy ] ) m_aInputData[ g_Config.m_ClDummy ].m_Direction = -1;
		if ( !m_aInputDirectionLeft[ g_Config.m_ClDummy ] && m_aInputDirectionRight[ g_Config.m_ClDummy ] ) m_aInputData[ g_Config.m_ClDummy ].m_Direction = 1;
...
// stress testing
		...
		if ( m_aInputData[ g_Config.m_ClDummy ].m_Direction != m_aLastData[ g_Config.m_ClDummy ].m_Direction ) Send = true;
		else if ( m_aInputData[ g_Config.m_ClDummy ].m_Jump != m_aLastData[ g_Config.m_ClDummy ].m_Jump ) Send = true;
		else if ( m_aInputData[ g_Config.m_ClDummy ].m_Fire != m_aLastData[ g_Config.m_ClDummy ].m_Fire ) Send = true;
		else if ( m_aInputData[ g_Config.m_ClDummy ].m_Hook != m_aLastData[ g_Config.m_ClDummy ].m_Hook ) Send = true;
		else if ( m_aInputData[ g_Config.m_ClDummy ].m_WantedWeapon != m_aLastData[ g_Config.m_ClDummy ].m_WantedWeapon ) Send = true;
		else if ( m_aInputData[ g_Config.m_ClDummy ].m_NextWeapon != m_aLastData[ g_Config.m_ClDummy ].m_NextWeapon ) Send = true;
		else if ( m_aInputData[ g_Config.m_ClDummy ].m_PrevWeapon != m_aLastData[ g_Config.m_ClDummy ].m_PrevWeapon ) Send = true;

		// send at at least 10hz
		if ( time_get( ) > LastSendTime + time_freq( ) / 25 ) Send = true;

		if ( m_pClient->m_Snap.m_pLocalCharacter && m_pClient->m_Snap.m_pLocalCharacter->m_Weapon == WEAPON_NINJA
		     && ( m_aInputData[ g_Config.m_ClDummy ].m_Direction || m_aInputData[ g_Config.m_ClDummy ].m_Jump || m_aInputData[ g_Config.m_ClDummy ].m_Hook ) )
			Send = true;
	}
...
{
	if ( !m_pClient->m_OxDummyControls.TakeControllDummy( ) )
	{
		...
		if ( !Force && ( !m_pClient->m_DummyInput.m_Direction && !m_pClient->m_DummyInput.m_Jump && !m_pClient->m_DummyInput.m_Hook && !m_pClient->m_DummyInput.m_Fire ) ) return 0;

		if ( g_Config.m_OxDummyAutoUnFreezeTee ) m_pClient->m_OxDummyControls.update_dummy_auto_shoot_unfreze( );
...
void CControls::OnRender( )
{
	...
	{
		// Keep track of ammo count, we know weapon ammo only when we switch to that
		// weapon, this is tracked on server and protocol does not track that
		...
		if ( m_aInputData[ g_Config.m_ClDummy ].m_Fire % 2 != 0 && m_pClient->m_Snap.m_pLocalCharacter->m_AmmoCount == 0 && m_pClient->m_Snap.m_pLocalCharacter->m_Weapon != WEAPON_HAMMER
		     && m_pClient->m_Snap.m_pLocalCharacter->m_Weapon != WEAPON_NINJA )
		{
			int w;
			for ( w = WEAPON_LASER; w > WEAPON_GUN; w-- )
			{
				if ( w == m_pClient->m_Snap.m_pLocalCharacter->m_Weapon ) continue;
				if ( m_aAmmoCount[ w ] > 0 ) break;
			}
			if ( w != m_pClient->m_Snap.m_pLocalCharacter->m_Weapon ) m_aInputData[ g_Config.m_ClDummy ].m_WantedWeapon = w + 1;
		}
	}

	// update target pos
	if ( m_pClient->m_Snap.m_pGameInfoObj && !m_pClient->m_Snap.m_SpecInfo.m_Active ) m_aTargetPos[ g_Config.m_ClDummy ] = m_pClient->m_LocalCharacterPos + m_aMousePos[ g_Config.m_ClDummy ];
	else if ( m_pClient->m_Snap.m_SpecInfo.m_Active && m_pClient->m_Snap.m_SpecInfo.m_UsePosition ) m_aTargetPos[ g_Config.m_ClDummy ] = m_pClient->m_Snap.m_SpecInfo.m_Position + m_aMousePos[ g_Config.m_ClDummy ];
	else m_aTargetPos[ g_Config.m_ClDummy ] = m_aMousePos[ g_Config.m_ClDummy ];
}

bool CControls::OnCursorMove( float x, float y, IInput::ECursorType CursorType )
{
	if ( m_pClient->m_Snap.m_pGameInfoObj && ( m_pClient->m_Snap.m_pGameInfoObj->m_GameStateFlags & GAMESTATEFLAG_PAUSED ) ) return false;
	...
	if ( CursorType == IInput::CURSOR_JOYSTICK && g_Config.m_InpControllerAbsolute && m_pClient->m_Snap.m_pGameInfoObj && !m_pClient->m_Snap.m_SpecInfo.m_Active )
	{
		...
		if ( Input( )->GetActiveJoystick( )->Absolute( &AbsX, &AbsY ) ) m_aMousePos[ g_Config.m_ClDummy ] = vec2( AbsX, AbsY ) * GetMaxMouseDistance( );
		return true;
	}

	float Factor = 1.0f;
	if ( g_Config.m_ClDyncam && g_Config.m_ClDyncamMousesens ) { Factor = g_Config.m_ClDyncamMousesens / 100.0f; }
	else
	{
		switch ( CursorType )
		{
	...
	if ( m_pClient->m_Snap.m_SpecInfo.m_Active && m_pClient->m_Snap.m_SpecInfo.m_SpectatorId < 0 ) Factor *= m_pClient->m_Camera.m_Zoom;
...
void CControls::ClampMousePos( )
{
	...
	else
	{
		...
		if ( MDistance < MouseMin ) m_aMousePos[ g_Config.m_ClDummy ] = normalize_pre_length( m_aMousePos[ g_Config.m_ClDummy ], MDistance ) * MouseMin;
		MDistance = length( m_aMousePos[ g_Config.m_ClDummy ] );
		if ( MDistance > MouseMax ) m_aMousePos[ g_Config.m_ClDummy ] = normalize_pre_length( m_aMousePos[ g_Config.m_ClDummy ], MDistance ) * MouseMax;
	}
}
